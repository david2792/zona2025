...
import puppeteer from 'puppeteer'; // Importar Puppeteer para la generación de PDF
import path from 'path';

class AgendamientoControlador {
    private readonly SECRET = "Secret Password";

    public async crearAgendamiento(req: Request, res: Response): Promise<void> {
        const pool = getPool();
        const { idprofesional, idpaciente, idusuario, fecha, hora, observacion, idestadoconsulta } = req.body;

        try {
            const [result] = await pool.query<ResultSetHeader>(
                'INSERT INTO citas (idprofesional, idpaciente, idusuario, fecha, hora, observacion, idestadoconsulta) VALUES (?,?,?,?,?,?,?)',
                [idprofesional, idpaciente, idusuario, fecha, hora, observacion.toUpperCase(), idestadoconsulta]
            );
    ...
    public async obtenerAgendamiento(req: Request, res: Response): Promise<void> {
        const pool = getPool();
        const { idprofesional, fecha } = req.body;

        try {
            const [row] = await pool.query("SELECT * FROM vagenda WHERE idprofesional = ? AND DATE(fecha) = ?", [idprofesional, fecha]);
    ...
    public async updateEstado(req: Request, res: Response): Promise<void> {
        ...
        try {
            const [result] = await pool.query<ResultSetHeader>('UPDATE citas SET idestadoconsulta = ? WHERE idcitas = ?', [idestadoconsulta, id]);
    ...
    public async updateEstadoCancelado(req: Request, res: Response): Promise<void> {
        const pool = getPool();
        const { id } = req.params;
...
public async generarPDF(req: Request, res: Response): Promise<void> {
    const pool = getPool();
    const { idprofesional, fecha } = req.body;

    try {
        // Obtener las citas para la fecha y el profesional dados, tipado explícito como RowDataPacket[]
        const [citas] = await pool.query<RowDataPacket[]>(`SELECT * FROM vagenda WHERE idprofesional = ? AND DATE(fecha) = ?`, [idprofesional, fecha]);
        ...
        if (Array.isArray(citas) && citas.length > 0) {
            ...
            await page.pdf({ path: 'reporte_citas.pdf', format: 'A4', printBackground: true });
...
export const agendamientoControlador = new AgendamientoControlador();
